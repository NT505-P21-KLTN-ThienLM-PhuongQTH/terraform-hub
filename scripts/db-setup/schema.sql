...
DROP SCHEMA IF EXISTS `mlflow_database` ;
CREATE SCHEMA IF NOT EXISTS `mlflow_database` DEFAULT CHARACTER SET utf8mb4 ;

DROP SCHEMA IF EXISTS `ghtorrent` ;
CREATE SCHEMA IF NOT EXISTS `ghtorrent` DEFAULT CHARACTER SET utf8mb4 ;

CREATE USER IF NOT EXISTS 'ghtorrent'@'%' IDENTIFIED BY 'ghtorrent';
GRANT ALL PRIVILEGES ON ghtorrent.* TO 'ghtorrent'@'%';
FLUSH PRIVILEGES;
...
ALTER TABLE users ADD COLUMN name VARCHAR(255) AFTER login;
ALTER TABLE users ADD COLUMN email VARCHAR(255) AFTER login;
ALTER TABLE users ADD COLUMN updated_at VARCHAR(255) AFTER login;
...
-- -----------------------------------------------------
-- Table `ghtorrent`.`workflows`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ghtorrent`.`workflows`;

CREATE TABLE IF NOT EXISTS `ghtorrent`.`workflows` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `github_id` BIGINT NOT NULL, -- workflow_id
  ...
  INDEX `idx_project_id_github_id` (`project_id` ASC, `github_id` ASC),
  CONSTRAINT `workflows_ibfk_1`
    FOREIGN KEY (`project_id`)
    REFERENCES `ghtorrent`.`projects` (`id`)
    ON DELETE CASCADE
...
-- -----------------------------------------------------
-- Table `ghtorrent`.`workflow_runs`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ghtorrent`.`workflow_runs`;

CREATE TABLE IF NOT EXISTS `ghtorrent`.`workflow_runs` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `github_id` BIGINT NOT NULL,         -- run_id
  ...
  `conclusion` VARCHAR(50) NULL DEFAULT NULL,
  `project_id` INT NOT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `github_id` (`github_id` ASC),
  INDEX `idx_workflow_id` (`workflow_id` ASC),
  INDEX `idx_commit_sha` (`head_sha` ASC),
  CONSTRAINT `workflow_runs_ibfk_1`
    FOREIGN KEY (`workflow_id`)
    REFERENCES `ghtorrent`.`workflows` (`id`)
    ON DELETE CASCADE,
  CONSTRAINT `workflow_runs_ibfk_2`
    FOREIGN KEY (`project_id`)
    REFERENCES `ghtorrent`.`projects` (`id`)
    ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8;